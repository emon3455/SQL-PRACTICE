-- Active: 1747626157296@@127.0.0.1@5432@ph
...
SELECT employee_name, salary, hire_date ,department_name FROM employees 
    INNER JOIN departments ON employees.department_id = departments.department_id

-- 2. **Group By Department with Average Salary**
SELECT d.department_name as "Department Name", avg(e.salary) as "Average Salary" from employees as e
    INNER JOIN departments as d ON e.department_id = d.department_id
        GROUP BY d.department_name 
            ORDER BY "Average Salary" DESC

-- 3. **Count Employees in Each Department**
SELECT count(e.employee_id) as "Employee Count", d.department_name  FROM employees as e
    INNER JOIN departments as d ON e.department_id = d.department_id
        GROUP BY d.department_name 
...
SELECT customer_id, count(*) as "Order Count", sum(total_amount) as "Total Amount Spent" FROM orders
    GROUP BY customer_id
        HAVING count(*) > 2

--  7: Find the total amount of orders placed each month in the year 2022.
SELECT EXTRACT(MONTH FROM order_date) as "Order Month", sum(total_amount) as "Total Amount", count(*) as "Total Number Of Order" FROM orders
